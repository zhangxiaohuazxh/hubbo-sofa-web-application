...
COPY hubbo-parent/hubbo-native-binding/build.gradle.kts hubbo-native-binding/
...
# 构建项目
RUN chmod +x ./gradlew && ./gradlew :hubbo-boot:bootJar -x test --no-daemon --parallel
...
HEALTHCHECK --interval=30s --timeout=3s --start-period=40s --retries=3 \
  CMD CMD curl -f http://localhost:8080/actuator/health || exit 1

# 设置 JVM 参数（可根据需要调整）
ENV JAVA_OPTS="-Xms256m -Xmx2g -XX:+UseContainerSupport"
ENTRYPOINT ["sh", "-c", "java $JAVA_OPTS -jar /app/libs/hubbo-boot-0.0.1.jar"]